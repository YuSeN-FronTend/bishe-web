<template>
    <div class="left-list">
        ...
        <div class="ll-list" v-if="activeName === 'chat'">
            <div class="list-child" v-for="item in chatList" :key="item.id" :id="'lc' + item.id"
        ...
        <div class="ll-list" v-else>
            <div class="list-child" v-for="item in companyArr" :key="item.id" :id="'lc' + item.id"
                ...
                <div class="lc-text">
                    <div class="lt-left">
                        <div class="lt-name">{{ item.name }}</div>
                        <!-- <div class="lt-info">您好我快递到哪了您好我快递到哪了您好我快递到哪了</div> -->
                    </div>
                    <div class="lt-right">
                        <!-- <div class="lr-time">9:13</div>
                            <div class="lr-count">1</div> -->
                    </div>
                </div>
            </div>
        </div>
...
import { onUnmounted, reactive, ref, onUpdated, onMounted, nextTick } from 'vue';
import { getCompany } from '../../../api/expressDelivery';
...
const rightName: any = ref('');
const companyArr: any = reactive([]);
...
onMounted(() => {
        ...
        if(sessionStorage?.getItem('activeName') === 'business'){
            chatHistory.value = [];
        }
})
onUpdated(() => {
...
async function selectCompany(item: any) {
...
function handleClick() {
    ...
    // }
    if (sessionStorage?.getItem('rightName')) {
        rightName.value = '';
    ...
    if (activeName.value === 'business') {
        chatHistory.value = [];
        getCompanyData();
        rightName.value = sessionStorage.getItem('rightName')
    } else if (activeName.value === 'chat') {
        chatHistory.value = JSON.parse(sessionStorage.getItem('chatInfo'));
        getList();
        rightName.value = sessionStorage.getItem('leftName');
    }
